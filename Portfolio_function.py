...
def compute_portfolio_period(Quotes, asset_weights, rebalance_periods='years'):
    ...
    if rebalance_periods == 'January':
        rebalance_dates = pd.bdate_range(start=Start, end=End, freq='BM')
        rebalance_dates = rebalance_dates[rebalance_dates.month==1]

    if rebalance_periods == 'February':
        rebalance_dates = pd.bdate_range(start=Start, end=End, freq='BM')
        rebalance_dates = rebalance_dates[rebalance_dates.month==2]

    if rebalance_periods == 'March':
        rebalance_dates = pd.bdate_range(start=Start, end=End, freq='BM')
        rebalance_dates = rebalance_dates[rebalance_dates.month==3]

    if rebalance_periods == 'April':
        rebalance_dates = pd.bdate_range(start=Start, end=End, freq='BM')
        rebalance_dates = rebalance_dates[rebalance_dates.month==4]

    if rebalance_periods == 'May':
        rebalance_dates = pd.bdate_range(start=Start, end=End, freq='BM')
        rebalance_dates = rebalance_dates[rebalance_dates.month==5]

    if rebalance_periods == 'June':
        rebalance_dates = pd.bdate_range(start=Start, end=End, freq='BM')
        rebalance_dates = rebalance_dates[rebalance_dates.month==6]

    if rebalance_periods == 'July':
        rebalance_dates = pd.bdate_range(start=Start, end=End, freq='BM')
        rebalance_dates = rebalance_dates[rebalance_dates.month==7]

    if rebalance_periods == 'August':
        rebalance_dates = pd.bdate_range(start=Start, end=End, freq='BM')
        rebalance_dates = rebalance_dates[rebalance_dates.month==8]

    if rebalance_periods == 'September':
        rebalance_dates = pd.bdate_range(start=Start, end=End, freq='BM')
        rebalance_dates = rebalance_dates[rebalance_dates.month==9]

    if rebalance_periods == 'October':
        rebalance_dates = pd.bdate_range(start=Start, end=End, freq='BM')
        rebalance_dates = rebalance_dates[rebalance_dates.month==10]

    if rebalance_periods == 'November':
        rebalance_dates = pd.bdate_range(start=Start, end=End, freq='BM')
        rebalance_dates = rebalance_dates[rebalance_dates.month==11]

    if rebalance_periods == 'December':
        rebalance_dates = pd.bdate_range(start=Start, end=End, freq='BM')
        rebalance_dates = rebalance_dates[rebalance_dates.month==12]
...
# years_2 = round(compute_portfolio_period(Quotes, [0.5, 0.5], 'years'), 2)
...
years     = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'years'), 2)
January   = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'January'), 2)
February  = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'February'), 2)
March     = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'March'), 2)
April     = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'April'), 2)
May       = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'May'), 2)
June      = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'June'), 2)
July      = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'July'), 2)
August    = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'August'), 2)
September = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'September'), 2)
October   = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'October'), 2)
November  = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'November'), 2)
December  = round(compute_portfolio_period(Quotes, [0.33, 0.33, 0.34], 'December'), 2)

time_series = merge_time_series(years, January)
time_series = merge_time_series(time_series, February)
time_series = merge_time_series(time_series, March)
time_series = merge_time_series(time_series, April)
time_series = merge_time_series(time_series, May)
time_series = merge_time_series(time_series, June)
time_series = merge_time_series(time_series, July)
time_series = merge_time_series(time_series, August)
time_series = merge_time_series(time_series, September)
time_series = merge_time_series(time_series, October)
